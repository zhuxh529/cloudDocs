 ...
 */
SourceImage::SourceImage(const BMP & image, int setResolution)
	: backingImage(image), resolution(setResolution)
{
	...
	resolution = min(backingImage.tellWidth(), backingImage.tellHeight());
...
{
	int width  = backingImage.tellWidth();
	int height = backingImage.tellHeight();
...
{
	if (backingImage.tellHeight() <= backingImage.tellWidth())
		return resolution;
	else
		return divide(resolution*backingImage.tellHeight(), backingImage.tellWidth());
}

int SourceImage::getColumns() const
{
	if (backingImage.tellWidth() <= backingImage.tellHeight())
		return resolution;
	else
		return divide(resolution*backingImage.tellWidth(), backingImage.tellHeight());
}
