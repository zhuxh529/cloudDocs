...
int main(int argc, const char** argv)
{
	...
	if (opts::help)
	{
		cout << "Usage: " << argv[0] << " background_image.bmp tile_directory/ [number of tiles] [pixels per tile] [output_image.bmp]" << endl;
		return 0;
	}
...
void makePhotoMosaic(const string & inFile, const string & tileDir, int numTiles, int pixelsPerTile, const string & outFile)
{
	...
	BMP result = mosaic->drawMosaic(pixelsPerTile);
...
vector<TileImage> getTiles(string tileDir)
{
	...
	for (size_t i = 0; i < imageFiles.size(); i++)
	{
		...
		cerr.flush();
		TileImage next(BMP(imageFiles.at(i)));
