...
if(!mazebody.empty()){
 mazebody.clear(); 
 solution.clear();// clear the vector
}
...
// to see if go to next cycle
  if(direction==1 && ifBottom(i)) continue;
  if(direction==0 && ifRight(i)) continue;

  int r, d;
  r=rightcell(i);
  d=downcell(i);
  if(direction==1 && set.find(i)==set.find(d)) continue;
  if(direction==0 && set.find(i)==set.find(r)) continue;
...
if(ifBottom(i)) mazebody[i].downwall=true;

if(ifRight(i)) mazebody[i].rightwall=true;
...
while(!q.empty())
 {
 temp=q.front();
 mazebody[temp].pass=true;
 x=getx(temp);
 y=gety(temp);
 q.pop();
 Di=mazebody[temp].dis;
 
 if(canTravel(x, y, 0) && mazebody[rightcell(temp)].pass==false)
 {
 r=rightcell(temp);
 mazebody[r].dis=Di+1;
 q.push(r);
 ...
 if(canTravel(x, y, 1) && mazebody[downcell(temp)].pass==false)
 {
	d=downcell(temp);
   mazebody[d].dis=Di+1;
    q.push(d);
 ...
 if(canTravel(x, y, 2) && mazebody[leftcell(temp)].pass==false)
 {
	l=leftcell(temp);
   mazebody[l].dis=Di+1;
    q.push(l);
 ...
 if(canTravel(x, y, 3) && mazebody[upcell(temp)].pass==false)
 {
	u=upcell(temp);
   mazebody[u].dis=Di+1;
    q.push(u);
